...
This script uses smallerado as the exam
%}
...
%[smallerado, smallerado_information] = generate_smallerado(isRandomization,isPlot);
...
population_labels_ground_truth_smallerado = cell(7,1);
for j = 1:7
    anno = fliplr(rot90(labels_upscale{j},2));
    for i = 1:length(smallerado_information.homes(:,1))
        if anno(smallerado_information.homes(i,2), smallerado_information.homes(i,1)) == 1
            population_labels_ground_truth_smallerado{j} = [population_labels_ground_truth_smallerado{j}; smallerado_information.homes(i,1), smallerado_information.homes(i,2), smallerado_information.homes(i,3)];
        end
    end
end
...
ground_truth_centers_smallerado = [];
for i = 1:7
    ground_truth_centers_smallerado = [ground_truth_centers_smallerado; round(mean(population_labels_ground_truth_smallerado{i}(:,1))) round(mean(population_labels_ground_truth_smallerado{i}(:,2)))];
end
...
if isPlot == 1
    ...
    plot(ground_truth_centers_smallerado(:,1),ground_truth_centers_smallerado(:,2),'c+','MarkerSize',8,'LineWidth',3),
    ...
    for i = 1:7
        subplot(3,3,i+1), 
        imshow(fliplr(rot90(ground_truth_image_upscale,2))), hold on,
        if ~isempty(population_labels_ground_truth_smallerado{i})
            scatter(population_labels_ground_truth_smallerado{i}(:,1), population_labels_ground_truth_smallerado{i}(:,2),'g.'), hold on,
            plot(ground_truth_centers_smallerado(i,1),ground_truth_centers_smallerado(i,2),'c+','MarkerSize',8,'LineWidth',3), hold on,
